...
func _ready():
	...
	connect("cambio_vida", get_parent().get_node("Interf"), "_on_cambio_vida")
...
func _physics_process(delta):
	...
	#anima los sprites
	if(movimiento.x != 0):
		$AnimatedSprite.animation = "der"
		$AnimatedSprite.flip_h = movimiento.x < 0
	elif(movimiento.y != 0):
		$AnimatedSprite.animation="frent"
	else:
		$AnimatedSprite.animation= "frent"
	...
	if(puede_disparar):
		if(velocidad.x != 0):
			...
			if($AnimatedSprite.is_flipped_h()):
				...
				dirbala.x = 80
			newbala.global_position = $posicionbala.global_position + dirbala
...
func _exit_tree() -> void:
	get_tree().root.get_node("tuto").emit_signal("gameover")
	
func _on_Invulner_timeout():
	puede_danar = true


func _on_disp_timeout():
	puede_disparar = true
